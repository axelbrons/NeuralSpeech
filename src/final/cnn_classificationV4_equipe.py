...
data_train = np.loadtxt("train_equipe.txt")
data_test = np.loadtxt("test_equipe.txt")

X_train = data_train.reshape((600,48,13))
X_test = data_test.reshape((60,48,13))

y_train = np.zeros((600,), dtype=np.float32)
y_train[50:100] = 1
y_train[100:150] = 2
y_train[150:200] = 0
y_train[200:250] = 1
...
y_train[550:600] = 2
#y_train[150:200] = 3
y_test = np.zeros((60,), dtype=np.float32)
y_test[5:10] = 1
y_test[10:15] = 2
y_test[15:20] = 0
y_test[20:25] = 1
...
def create_model():
    ...
    model = tf.keras.Sequential([
        ...
        tf.keras.layers.Dense(3, activation='softmax', use_bias = IS_BIASED, kernel_initializer=initializer)  # Output layer (10 classes)
    ])
    return model
